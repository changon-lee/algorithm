...
KNAPSACK* solve_knapsack_problem(double arr[], int arr_len){
        ...
        //find path
        int *index = (int*)malloc(sizeof(int*)*count_index);
        if(!index){
                fprintf(stderr,"Knapsack index array memory allocation failed.");
                exit(0);
        }
        ...
        if(!k) {
                fprintf(stderr,"Knapsack Memory allocation failed.");
                exit(0);
        }
        k->index = index;
        k->result = result;
...
double* get_input_items(int n){
        ...
        if(!arr){
                fprintf(stderr,"failed to get input items");
                exit(0);
        }
...
void enlarge_array(int *s, double** ar){
        ...
        if(!temp){
                fprintf(stderr,"Enlarge Array Memory allocation failed.");
                exit(0);}
        for(i=0;i<*s;i++){
                temp[i]=(*ar)[i];
        }
...
KNAPSACK* knapsack_initialize(int* index, int count, double result){
        KNAPSACK *k = (KNAPSACK*)malloc(sizeof(KNAPSACK*));
        if(!k){
                fprintf(stderr,"Knapsack Initialization Failed.");
                exit(0);
        }
        k->index = index;
        k->count_index = count;
...
int main(){
        ...
        printf("Input file for 1st, 2nd, 3rd Problem : ");
        scanf("%s",string);
        ...
        if(!arr){
                fprintf(stderr,"Initial Array Memory allocation failed.");
                exit(0);}
        while(1){
        ...
        if(!third_array){
                fprintf(stderr,"Third Array Memory Allocation Failed.");
                exit(0);
        }
        for(i=0;i<arr_len;i++){
                third_array[i] = arr[i];
        ...
        printf("Input file for 4th Problem : ");
        scanf("%s", string);
        ...
        if(!arr){
                fprintf(stderr,"4th Array Memory Allocation Failed.");
                exit(0);
        }
